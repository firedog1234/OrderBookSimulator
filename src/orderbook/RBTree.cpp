...
RBTree::RBTree() {
    node_pool.reserve(SIZE);
}

RBNode::RBNode(Order& order) : parent(nullptr), left(nullptr), right(nullptr), isRed(true) {
...
void RBTree::fixInsert(RBNode* node) {
    ...
    if (uncle != nullptr && parent->isRed == true && uncle->isRed == true) {
        parent->isRed = false;
        uncle->isRed = false;
        grand->isRed = true;
        fixInsert(grand);
    ...
    else if ((uncle == nullptr && parent->isRed == true) || (uncle->isRed == false && parent->isRed == true)) {
        // CASE A: Parent is left child, Node is left child (Left-Left)
    ...
    while (node != nullptr) {
        if (price < node->level.priceLevel) {
            ...
            for (auto it = orders.begin(); it != orders.end(); ++it) {
                if ((*it)->id == order.id) {
                    orders.erase(it); //https://www.geeksforgeeks.org/cpp/dequeclear-dequeerase-c-stl/
                    node->level.totalQuantity -= order.quantity;
                    break;
                }
            }
        }
        // YO ADD YOUR DELETE NODE IF YOUR THING THE UM THE ORDER DEQUE IS EMPTY YOU FEEL AFTER YOU IMPLEMNT DELETE NOTE TO SELF
        //IGU
        if (node->level.orders.empty()) {
            remove(node);
        }
    }
}
...
void RBTree::fixDelete(RBNode* x, RBNode* parent) {
    ...
    while ((cur != root) && (cur == nullptr || cur->isRed == false)) {
        if (curParent == nullptr) break;
        ////need

        RBNode* sibling = (curParent->left == cur) ? curParent->right : curParent->left;

        //CASE ONE: sibling is red
        if (sibling && sibling->isRed) {
            sibling->isRed = false;
            curParent->isRed = true;
            if (curParent->left == cur) rotateLeft(curParent);
            else rotateRight(curParent);
            // update sibling after rotation cuz wedk if its cause its prolly not the same
            sibling = (curParent->left == cur) ? curParent->right : curParent->left;
        }

        bool leftBlack = (!sibling || !sibling->left || sibling->left->isRed == false);
        bool rightBlack = (!sibling || !sibling->right || sibling->right->isRed == false);
        ...
        if (sibling && leftBlack && rightBlack) {
            ...
            if (curParent->left == cur) {
                ...
                if (sibling) sibling->isRed = curParent->isRed;
                curParent->isRed = false;
                if (sibling && sibling->left) sibling->left->isRed = false;
...
//my logic come from this dudes video: https://www.youtube.com/watch?v=TlfQOdeFy0Y&t=655s
//https://www.youtube.com/watch?v=iw8N1_keEWA&t=171s
//this helped me fix my delete stuff https://github.com/msambol/dsa/blob/master/trees/red_black_tree.py#L229
